...
```
{
  ...
  # sentences in the document, each sentence is a list of tokens,这里有个问题，sentences虽然是列表形式，
  但是后面的span仍然是从0开始不重置。看看是不是又在list中重置。 done
  这里是sentence_list列表的索引和ner的索引是直接对应的。WikiEvents中没有，要用sent_idx对应
  长sentence的问题出在这里，需要对后续所有的sent_idx做改变，才能对应上新的列表的idx
  "sentences": [
    [...],
    [...],
  ...
  "ner": [
    ...
    # fixme 在WikiEvents中start到end 是end+1，改回这种形式只需要end-1
    ...,
  ],
